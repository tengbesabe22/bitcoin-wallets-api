...
function generateP2SHWallet(n, m, publicKeys) {
    ...
    if (publicKeys.length > m) {
        throw new BadError_1.BadError('Public key count cannot be more than m');
    }
    if (n > publicKeys.length) {
        throw new BadError_1.BadError('Invalid n, cannot be greater than m');
    }
    ...
    try {
        wallet = bitcoin.payments.p2sh({
            redeem: bitcoin.payments.p2ms({ m: Number(n), pubkeys: pubkeys, network: bitcoinNetwork[BITCOIN_NETWORK] }),
        });
    }
    catch (BitcoinError) {
...
function generateBip49Wallet(mnemonic, initialPath) {
    ...
    var path;
    try {
        path = wallet_utils_1.standardizePath(initialPath, "49'", COIN_TYPE);
    }
    catch (PathError) {
